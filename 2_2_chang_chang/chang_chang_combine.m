...
sigma1_range = linspace(0, 400, 100);
...
chang_fiber1 = (sigma1.^2)/(Xt.^2) + 0.5*(tau12.^2)/(Sc.^2);

isosurface_threshold = 1;
%
h1 = patch(isosurface(sigma1, sigma2, tau12, chang_fiber1, isosurface_threshold));
hold on;
isonormals(sigma1, sigma2, tau12,chang_fiber1,h1)
...
sigma2_range = linspace(-600, 600, 100);
tau12_range = linspace(-100, 100, 100);

% Create a meshgrid for stress components
[sigma1, sigma2, tau12] = meshgrid(sigma1_range, sigma2_range, tau12_range);

% equastion from Paper: http://dx.doi.org/10.1016/j.engfailanal.2013.07.001
chang_fiber2 = (sigma1.^2)/(Xc.^2);

isosurface_threshold = 1;
%
h2 = patch(isosurface(sigma1, sigma2, tau12, chang_fiber2, isosurface_threshold));
hold on;
isonormals(sigma1, sigma2, tau12,chang_fiber2,h2)
...
isosurface_threshold = 1;
%
h3 = patch(isosurface(sigma1, sigma2, tau12, chang_matrix1, isosurface_threshold));
hold on;
isonormals(sigma1, sigma2, tau12,chang_matrix1,h3)
...
isosurface_threshold = 1;
%
h4 = patch(isosurface(sigma1, sigma2, tau12, chang_matrix2, isosurface_threshold));
hold on;
isonormals(sigma1, sigma2, tau12,chang_matrix2,h4)
...
SH_tau12 = fv4/sqrt(beta); % Replace with your experimental value
...
print(gcf,'-dtiffn','chang_combine_1')
view([0 90]);
print(gcf,'-dtiffn','chang_combine_2')
view([0 1]);
colorbar off
print(gcf,'-dtiffn','chang_combine_3')
view([-90 1]);
colorbar off
print(gcf,'-dtiffn','chang_combine_4')